...
template <typename T> class UCBAgent : public AgentInterface<T> {
private:
  // <estimation value, selection>
  using Key_t = std::pair<double, int>;
  // estimation updates the arm's value interately, estimation[i] indicates the
  // ith arm's value.
...
public:
  UCBAgent(std::string &&name, double _c = 2.0f)
      : AgentInterface<T>(std::forward<std::string &&>(name)), c(_c) {}
...
private:
  void UpdateEstimations(int selection, double reward) {
    ...
    estimation[selection] +=
        1.0 / choose_cnt[selection] * (reward - estimation[selection]);
    ...
    for (size_t i = 0; i < estimation.size(); i++) {
      ...
      if (choose_cnt[i] == 0)
        ...
        ucb_value = estimation[i] +
                    c * std::sqrt(std::log(total_steps) / choose_cnt[i]);
