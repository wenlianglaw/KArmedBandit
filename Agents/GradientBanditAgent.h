 ...
 *
 * If reward is higher than the average reward, the probablity of taking this
 * action is increased in the future.  The Non taken actions move in the
 * opposite way.
 *
 ...
 */
template <typename T> class GradientBanditAgent : public AgentInterface<T> {
private:
  std::vector<double> H;
...
public:
  GradientBanditAgent(std::string &&name, double _alpha)
  ...
  void Init(Testbed<T> *t) override {
    ...
    sumOfEToH = std::accumulate(H.begin(), H.end(), 0.0f,
